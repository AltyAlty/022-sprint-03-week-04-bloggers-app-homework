...
@injectable()
export class OptionalAccessTokenGuardMiddleware {
  constructor(
  ...
  public async execute(req: Request<{}, {}, {}, {}>, res: Response, next: NextFunction): Promise<void | Response> {
    /*Получаем значение заголовка "authorization" их запроса.*/
    const authorizationHeader: string | undefined = req.headers['authorization'];

    /*Если в заголовках запроса нет заголовка "authorization", то разрешаем дальнейшее выполнение запроса при помощи
    функции "next()".*/
    ...
    /*Если тип авторизации не "Bearer", то разрешаем дальнейшее выполнение запроса при помощи функции "next()".*/
    if (authType !== 'Bearer') {
      next();
      return;
    }

    /*Если тип авторизации "Bearer", то получаем имя устройства пользователя из запроса.*/
    ...
    /*Если IP-адрес пользователя был найден, то просим адаптер "jwtAdapter" верифицировать AT.*/
